...
data class ModelState(val corpus: List<LinearState>,
    ...
    fun addDataRows(dataRowsToAdd: List<DataRowState>) = copy(corpus = corpus.plus(dataRowsToAdd))
    fun removeDataRows(dataRowsToRemove: List<DataRowState>) = copy(corpus = corpus.minus(dataRowsToRemove))
    fun addGateKeepers(gateKeepersToAdd: Party) = copy(participants = participants.plus(gateKeepersToAdd))
